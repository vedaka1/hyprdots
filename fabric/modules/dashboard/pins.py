...
class Cell(Gtk.EventBox):
    ...
    def update_display(self):
        ...
        if self.content is None:
            ...
            if self.content_type == 'file':
                ...
                label = Label(
                    name='pin-file',
                    label=os.path.basename(self.content),
    ...
    def select_file(self):
        dialog = Gtk.FileChooserDialog(
            title='Select File', parent=self.get_toplevel(), action=Gtk.FileChooserAction.OPEN
        )
...
class Pins(Gtk.Box):
    ...
    def save_state(self):
        state = []
        for cell in self.cells:
            state.append({'content_type': cell.content_type, 'content': cell.content})
        try:
