...
class KanbanColumn(Gtk.Frame):
    ...
    def setup_dnd(self):
        self.listbox.drag_dest_set(
            Gtk.DestDefaults.ALL, [Gtk.TargetEntry.new('UTF8_STRING', Gtk.TargetFlags.SAME_APP, 0)], Gdk.DragAction.MOVE
        )
...
class Kanban(Gtk.Box):
    ...
    def __init__(self):
        ...
        self.columns = [KanbanColumn('To Do'), KanbanColumn('In Progress'), KanbanColumn('Done')]
    ...
    def save_state(self):
        state = {'columns': [{'title': col.title, 'notes': col.get_notes()} for col in self.columns]}
        try:
