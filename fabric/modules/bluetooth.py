...
class BluetoothDeviceSlot(CenterBox):
    ...
    def on_changed(self, *_):
        self.connection_label.set_label('Connected' if self.device.connected else 'Disconnected')
...
class BluetoohConnections(Box):
    def __init__(self, **kwargs):
        ...
        self.client.connect(
            'notify::enabled',
            lambda *_: self.toggle_button.set_label('Bluetooth ' + ('ON' if self.client.enabled else 'OFF')),
        )
        self.client.connect(
            'notify::scanning',
